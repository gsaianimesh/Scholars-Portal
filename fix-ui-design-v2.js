...
const headerRegex = /<div className="flex items-center gap-4">[\s\S]*?<\/div>\n      <\/div>\n\n      \{\/\* Reschedule Dialog \*\/}/;
const newHeader = `<div>
...
const briefingRegex = /\{!isPast && preMeetingContext && \([\s\S]*?<\/CardContent>\n        <\/Card>\n      \)\}/;
const newBriefing = `{!isPast && preMeetingContext && (
...
code = code.replace(briefingRegex, newBriefing);
